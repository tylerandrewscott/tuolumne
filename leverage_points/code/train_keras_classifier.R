...
auto_coded_df = fread('scratch/leverage_points/auto_trainers.csv',encoding = 'UTF-8',stringsAsFactors = F)
#auto_coded_df = auto_coded_df[auto_coded_df$found==1,]
auto_y_matrix = auto_coded_df[,grepl('X_',colnames(auto_coded_df)),with=F] %>% as.matrix()
...
tokenizer <- text_tokenizer(num_words=MAX_NUM_WORDS,lower = TRUE)
...
save_text_tokenizer(tokenizer, "model_objects/leverage_points/tokenizer")
...
keras::save_model_hdf5(model,filepath = 'model_objects/leverage_points/topic_classifier_fixed_waterclimatedrought.h5',include_optimizer = TRUE)
