...
auto_coded_df = data.table::fread('input/scratch/auto_trainers.csv')
...
tokenizer <- text_tokenizer(num_words=MAX_NUM_WORDS,char_level = F)
...
emb_layer = layer_embedding(object = seq_input,embeddings_initializer = 'uniform',output_dim = EMBEDDING_DIM,
                              input_length = MAX_SEQUENCE_LENGTH,input_dim = length(word_index)+1,weights = list(embedding_matrix),
                            trainable = TRUE)
...
output =  layer_dense(object = h,units = 7,activation = 'sigmoid')
...
keras::save_model_hdf5(model,filepath = 'model_objects/topic_classifier_fixed.h5',include_optimizer = TRUE)
