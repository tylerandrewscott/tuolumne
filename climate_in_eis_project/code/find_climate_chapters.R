...
full_rec = fread('../eis_documents/enepa_repository/meta_data/eis_record_detail.csv',encoding = 'UTF-8')
...
full_rec$Climate_Section_File = NA
full_rec$Climate_Section_Start_Page = NA
full_rec$Climate_Section_End_Page = NA
...
exec_file = grepl('CLIMATE|EMISSIONS|GREENHOUSE',fl2)&!grepl('Appendix|APPENDIX|Errata',fl2)
full_rec$Climate_Section_File = fl2[exec_file][match(full_rec$EIS.Number,fl_id2[exec_file])]

require(textclean)


temp = full_rec[,.(Title,EIS.Number,DEIS_NUMBER,Climate_Section_File,Climate_Section_Start_Page,Climate_Section_End_Page)]
temp$YEAR = str_extract(temp$EIS.Number,'^[0-9]{4}')
temp$DOCUMENT = full_rec$Document[match(temp$EIS.Number,full_rec$EIS.Number)]


removes = which(temp$Climate_Section_End_Page-temp$Climate_Section_Start_Page>100)
temp$Climate_Section_File[removes]<-NA
temp$Climate_Section_Start_Page[removes]<-NA
temp$Climate_Section_End_Page[removes]<-NA
summary(temp$Climate_Section_End_Page)

fl = fl[grepl('pdf$|PDF$',fl)]

require(pdftools)

climate_words = c('climate','greenhouse','emissions','cumulative','resilience','hazard','sea level rise','')
climate_regex = paste(c(str_to_title(climate_words),str_to_upper(climate_words)),collapse = '|')

#go_get = which(temp$EIS.Number %in% full_rec$EIS.Number[full_rec$Agency=='Forest Service'] & is.na(temp$Climate_Section_File))
for(i in which(is.na(temp$Climate_Section_File))[1:50]){
  if(is.na(temp$Climate_Section_File[i])){
    print(i)
    i = which(temp$EIS.Number=='20140302')
    rm(temp_flist);rm(temp_read_list);rm(toc_list)  
    temp_flist  = fl2[fl_id2 %in% temp$EIS.Number[i]]
    ...
    need_next_level = sapply(childs,length)==1|!grepl(climate_regex,childs)
     childs[need_next_level] <- lapply(which(need_next_level),function(x) sapply(toc_list[[x]]$children[[1]]$children,function(u) u$title))
    has_exec_sum = sapply(childs,function(x) any(grepl(climate_regex,toupper(x))))
    if(!any(has_exec_sum)&!is.na(temp$DEIS_NUMBER[i])){
      temp_flist  = fl[fl_id %in% temp$DEIS_NUMBER[i]]
    if(any(grepl(climate_regex,temp_flist))){temp$Climate_Section_File[i]<-temp_flist[grepl(climate_regex,temp_flist)];print('found whole draft');next}
      toc_list = lapply(seq_along(temp_flist),function(f){
        dfile   = temp_flist[f]
        tryCatch(pdf_toc(dfile),error = function(e) NULL)})
      childs = lapply(seq_along(toc_list),function(t) sapply(toc_list[[t]]$children,function(x) x$title))
      has_exec_sum = sapply(childs,function(x) any(grepl(climate_regex,x)))
    }
    ...
    if(sum(has_exec_sum)>1){has_exec_sum <- has_exec_sum & sapply(childs,function(x) any(grepl(climate_regex,x)))}
    ...
    if(sum(has_exec_sum)!=1){print(paste('found',sum(has_exec_sum),'sections'));next}else{
      ...
      is_exec_section = grepl(climate_regex,toupper(childs))
      which_pages = which(sapply(head_pages,function(x) any(grepl(climate_regex,x))&
     ...
     # which_next_pages = which(sapply(head_pages,function(x) any(grepl(toupper(next_section),x))))
      ...
      if(which_pages<where_table_contents_starts&where_table_contents_starts<which_next_pages){
        print('success')
        temp$Climate_Section_File[i]<-dfile
        temp$Climate_Section_Start_Page[i] <- which_pages
        temp$Climate_Section_End_Page[i] <- where_table_contents_starts-1
      }else{
      if(min(which_pages) < min(which_next_pages)){
        print('success')
        temp$Climate_Section_File[i]<-dfile
        temp$Climate_Section_Start_Page[i] <- which_pages
        temp$Climate_Section_End_Page[i] <- which_next_pages-1
      }else{
        print('Bad find')
      }
      }
...
temp[1:50,]$Climate_Section_File
#fwrite(temp,'exec_summary_locations.csv')


test = pdf_toc(temp_flist)
sapply(test$children,function(x) x$title)
sapply(test$children[[5]]$children,function(x) x$title)

temp_flist

temp[EIS.Number=='20130046']

full_rec[EIS.Number=='20130046']
full_rec[Agency == 'Federal Energy Regulatory Commission',][8,]



tt = readRDS('scratch/boilerplate/big_text_files/big_eis_text.rds')
phrase = 'incremental physical impacts'

ph = grep(phrase,tt$text)
dim(tt)
...
#   full_rec$Climate_Section_File[i] <- temp_flist[mx]
#   }
# },cl = 4)
# 
# 
# 
# table(is.na(full_rec$Climate_Section_File))
# 
# 
# 
#   
...
# 
# which(is.na(full_rec$Climate_Section_File))[447]
...
#   if(!is.na(full_rec$Climate_Section_File[i])){temp_flist    = full_rec$Climate_Section_File[i]}else{
#   temp_flist  = fl[fl_id %in% full_rec$EIS.Number[i]]}
#  
#   #flist = flist[gsub('.pdf','.txt',flist) %in% readable]
#   temp_read_list = lapply(seq_along(temp_flist),function(f){
...
# 
# require(pdftools)
# test = '../eis_documents/enepa_repository/documents/2015/20150280_00_OMSF_FEIS_Climate_Section.pdf'
# tt = pdf_text(test)
# 
# 
# es_regex = '^Summary of the Final EIS|^Summary$|^SUMMARY$|^Executive Summary|^EXECUTIVE SUMMARY|\nSummary$|\nSUMMARY$|\nAbstract|\nABSTRACT|\nExecutive Summary|\nEXECUTIVE SUMMARY|Summary of Plan|Executive Summary$|EXECUTIVE SUMMARY$'
