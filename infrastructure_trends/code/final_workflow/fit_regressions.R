...
pcols = grep('P',colnames(class),value = T)

class[,(pcols):=lapply(.SD,function(x) (x>0.5)+0),.SDcols = pcols]
...
######
library(tigris)
county_tigris = counties(class = 'sf',year = 2016)
...
rural_urban = readxl::read_excel('input/ruralurbancodes2013.xls') %>% dplyr::select(-County_Name) %>% dplyr::rename(CFIPS = FIPS)
...
cog$Num_Local_Govs = replace_na(cog$special_districts, 0) + replace_na(cog$municipal,0)+replace_na(cog$town_township, 0)
...
ch_values = readRDS(paste0(scratch_loc,'scratch/descriptive_paper/county_ch_data.RDS'))
...
saveRDS(object = mod_list,file = paste0(scratch_loc,'scratch/descriptive_paper/mods_functional_form.RDS'))
