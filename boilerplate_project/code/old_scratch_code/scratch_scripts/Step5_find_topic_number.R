...
simiters = rep(k_list,each = 10)
n_iter = 100
best_k = data.table(sim = as.numeric(),k = as.numeric(),perplexity = as.numeric())
...
model = LDA$new(k, doc_topic_prior = 0.1, topic_word_prior = 0.01)
doc_topic_distr =
  model$fit_transform(dtm, n_iter = sim, n_check_convergence = 1,
                      convergence_tol = -1, progressbar = FALSE,n_threads = 4)
topic_word_distr_10 = model$topic_word_distribution
perp = perplexity(dtm, topic_word_distr_10, doc_topic_distr)
best_k = rbind(best_k,data.table(sim = sim,k = k,perplexity = perp))}

print(best_k)
saveRDS(best_k,paste0(floc,'topic_k_search_sim_results.RDS'))
