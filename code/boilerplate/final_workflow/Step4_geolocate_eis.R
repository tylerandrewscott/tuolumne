...
grep('20180023',full_tlist$File,value = T)
...
clusterExport(cl , list('county_tigris','state_tigris','ngaz','top40pages'))

fls = unique(top40pages$File)

fips_matches = foreach(i = fls) %dopar% {
  id = str_remove(i,'_.*')
  text = top40pages$text[top40pages$File==i]
...
saveRDS(geomatches,'../bucket_mount/tuolumne/scratch/boilerplate/geo_temp.rds')


# 
# geomatches = readRDS('../bucket_mount/tuolumne/scratch/boilerplate/geo_temp.rds')
# 
...
# 
projects[EIS.Number %in% have_docs & !EIS.Number %in% have_geo]
full_tlist$text[grepl('20140374',full_tlist$File)]
documents[EIS.Number=='20180023']

test = paste(full_tlist$text[grepl('20140374',full_tlist$File)],collapse = ' ')
pars = spacy_parse(test,entity = T,dependency = T,full_parse = TRUE,lemma = F,pos = F,nounphrase = T)
ents = entity_consolidate(pars)
filter_ents = ents[ents$entity_type%in%c('GPE','LOC',"FAC"),]
states = unique(filter_ents$token[filter_ents$token %in% gsub('\\s','_',state.name)])
if(length(states)==0){states = gsub("'",'',unique(filter_ents$token[gsub("'",'',filter_ents$token) %in% state.name]))}
...
# 
# 
# fips_matches = foreach(i = top40pages$text[str_remove(top40pages$File,'_.*') %in% '20180293'],j = top40pages$File[str_remove(top40pages$File,'_.*') %in% '20180293']) %dopar% {
#   id = str_remove(j,'_.*')
...
# 
# projects[!EIS.Number %in% geomatches$PROJECT_ID & EIS.Number %in% str_remove(unique(full_tlist$File),'_.*'),]
# 
# 
# table(ifelse(projects$EIS.Number %in% str_remove(full_tlist$File,'_.*'),'File','No File'),projects$EIS.Number %in% geomatches$PROJECT_ID)
# 
# foreach(i in full_tlist$text[grepl('20180293',full_tlist$File)]){
# id = '20180293'
# pars = spacy_parse(i,entity = T,dependency = T,full_parse = TRUE,lemma = F,pos = F,nounphrase = T)
# ents = entity_consolidate(pars)
# filter_ents = ents[ents$entity_type%in%c('GPE','LOC',"FAC"),]
...
# 
# 
# table(projects$EIS.Number %in% old_fips_matches$PROJECT_ID,projects$EIS.Number %in% geomatches$PROJECT_ID)
