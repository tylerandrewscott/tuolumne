...
stringcombos = c('Climate Change','Hazard','Sea Level Rise','Drought','Greenhouse Gas','Global Warming') 
stc = paste(c(stringcombos,toupper(stringcombos)),collapse='|')

flist = list.files('~/Box/eis_documents/enepa_repository/text_as_datatable/',full.names = T,recursive = T)
flist2 = list.files('~/Box/eis_documents/agency_nepa_libraries/usfs/text_as_datatable/',full.names = T,recursive = T)
flist3 = list.files('~/Box/eis_documents/agency_nepa_libraries/blm/text_as_datatable/',full.names = T,recursive = T)
flist4 = list.files('~/Box/eis_documents/agency_nepa_libraries/doe/text_as_datatable/',full.names = T,recursive = T)
...
prep_file = 'scratch/climate_in_nepa/climate_pages.rds'
...
phrase = 'Because we cannot determine the Project’s incremental physical impacts'
...
list_of_preparers = lapply(proj_agencies,function(f) {
  ...
  temp_dt = rbindlist(lapply(temp_flist,function(j) {
    ...
    temp$File = j
    temp}))
  temp_prep = temp_dt[grepl(stc,text)&!grepl('Table of Contents|TABLE OF CONTENTS|DOCUMENT ORGANIZATION|\\.{8,}|Literature Cited|REFERENCES|LITERATURE CITED',text),]
  temp_prep}},cl = 6))
})
