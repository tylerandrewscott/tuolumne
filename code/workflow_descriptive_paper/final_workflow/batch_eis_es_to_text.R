...
require(textclean)
library(pdfsearch) 
#epa = fread('input/epa_master_repository/eis_record_detail_coded.csv')

#fls = list.files('../eis_documents/enepa_repository/meta_data/',pattern = 'eis_record_detail',full.names = T)

#epa = rbindlist(lapply(fls,fread,na.strings = c('','NA'),use.names = T,fill = T)
#epa = epa[!duplicated(epa),]
epa = fread('../eis_documents/enepa_repository/meta_data/eis_record_detail.csv')
epa$Title = gsub("[^[:print:]]", "", epa$Title)
#epa2 = epa2[!epa2$EIS.Number %in% epa$EIS.Number,]
#epa = rbind(epa,epa2,use.names=T,fill = T)
#epa2 = fread('../eis_documents/enepa_repository/meta_data/eis_record_detail.csv')
#epa2 = epa2[!epa2$EIS.Number %in% epa$EIS.Number,]
#epa = rbind(epa,epa2,use.names = T,fill = T)


feis = epa[Document=='Final',]
feis$Title = textclean::replace_non_ascii(feis$Title)
feis$YEAR = str_extract(feis$EIS.Number,'^[0-9]{4}')
feis = feis[YEAR<2020,]
feis$ADOPTED = grepl('^ADOPT',feis$Title)
feis$WITHDRAWN = grepl('^WITHD|^Withdrawn',feis$Title)
feis$VOID = grepl('^VOID|VOIDED',feis$Title)
feis$CANCELLED = grepl('^CANCELL',feis$Title)
feis$KEEP =  !feis$ADOPTED & !feis$WITHDRAWN & !feis$CANCELLED & !feis$VOID
feis$SUBSAMPLE = feis$YEAR %in% 2013:2019


fwrite(feis,'feis_record.csv')


dim(feis[YEAR %in% 2013:2019&KEEP==T,])
...
fdt$Area[grepl(paste(paste0('\\b',c('Ski','Resort','Golf','Boating','Marina','Guest Ranch','Snowmobile','Recreation Complex','Entertainment Arena','Trail Easement',
                                    ...
                                    'Trails Management Plan','Burning Man',
                                    ...
                                    'Park Comprehensive Design Plan','Heritage Center',"Park Project  Construction and Operation",'Authorize Harvest','Gondola',
                                    ...
                                    'San Gabriel River Confluence With Cattle Canyon Improvements Project','National Memorial Designation',
...
fdt$Area[grepl(paste(paste0('\\b',c('Levee','Lock','Reservoir Expansion','Dam Raise','Groundwater Development','Dam Safety','Dam and Reservoir',
                                    ...
                                    'Delta Islands and Levees','East Side Coastal Resiliency (ESCR)',
                                    ...
                                    'Lower Basin Shortages','Instream Flow','Water Storage Feasibility','Aquifer Storage','Spillway Replacement','201 Facilities Plan',
                                    ...
                                    'Shoreline Management Initiative','Conjunctive Use Project','Watershed Project','Watershed Plan','Joint Water Pollution Control Plant','Newlands Project Operating Criteria',
                                    ...
                                    'Freshwater Diversion Project','Water Resources Study','Moffat Collection System Project','West Sacramento Project','Jenning Randolph Lake','Dam Operation',
                                    'Area Flood Risk Management','Reduction of Flood Damages','Flood Protection','Hurricane Protection','River System Operation Review',"Mendota Pool Group",
                                    'Recycled Water Program','Regional Water System','Operating Agreement for the Rio Grande Project','Shallow Flow Equalization Basin','Resilient Bridgeport',
                                    'Dam Operation  Implementation','Water Recycling','Safety of Dams','Dam and Lake Project','Agricultural Water','Drainage Basin Improvement','East Side Coastal Resiliency',
...
fdt[Title=='High Desert Corridor']
...
fdt$Area[grepl(paste(paste0('\\b',c('Transmission','Energy','Power Plant','Pipeline','Hydroelectric','Hydropower','Power Station','Hydro Project','Cogeneration',
                                    ...
                                    'Floating Production Storage and Offloading Systems','Hermiston Power Project',
...
fdt$Area[grepl(paste(paste0('\\b',c('Mine','Mining','Quarries','Underground Mines','Tailings Impoundment','Uranium Project','Gold Project','Greater Phoenix Project','Uranium Mill Trailings','Mackay Optimization Project',
                                    'Mineral Withdrawal','Hardrock Mineral','Tailings Disposal','Uranium Recovery','Leach Uranium','Mineral Material Sales','Uranium Leasing','Potash Project','Deep South Expansion Project',
                                    'Trailings Reclamation','Phosphate Leasing','Uranium Mill Tailings','Locatable Mineral Operations','Copper Leach Project','In-Situ Recover','Federal Mineral Estate','Locatable  Mineral Operations',
...
fdt$Area[grepl(paste(paste0('\\b',c('Highway','US-[0-9]{1,3}','I-[0-9]{1,3}','SR [0-9]{1,3}','S\\.R\\. [0-9]{1,3}','U\\.S\\. Route','US Route','US [0-9]{1,3}','Interstate [0-9]{1,3}','Bridge','Rapid Transit','Bridge Plaza',
                                    ...
                                    'Wharf Extension','Six County Association of Governments Construction and Operation',"Northwest SR-138 Corridor",
                                    ...
                                    'Ship Channel Improvement','Small Boat Harbor Facility','Cruise Terminal Project','Container Handling Facility','Permanent Terminal','Locks',
                                    ...
                                    'Deepwater Port','Dredging','Dredged','Navigation','Road Construction','Railroad Line','Freeway Bypass','Railroad Project','Connecting Channels','Ship Channel',
                                    'Transportation','Travel Management','Transit System','Transit Improvements'),'\\b'),collapse='|'),fdt$Title)] <- 'Transportation'

fdt$Area[is.na(fdt$Area)&fdt$Agency%in%c('Federal Highway Administration','California Department of Transit','Federal Transit Administration','California Department of Transportation')&grepl('North|South|East|West|Reconstruction|Relocation|Interstate|Improvement|Right-of-Way|Arterial|Loop|Crossing|Section|Road|Widening|Avenue|Route|Corridor|Bypass|Boulevard|Viaduct|Road Realignment|Seismic Improvement|Road Improvement|Freeway|Extension|Connector|Roadways',fdt$Title)] <- 'Transportation'
...
fdt$Area[grepl(paste(paste0('\\b',c('Laboratory','Research','Experimental','Observatory','Rocket','Medical Isotopes',
                                    ...
                                    'Doppler Weather Radar','Antarctica','Scientific Ocean Drilling','Savannah River Site Continued Operation',
                                    'Genetic Resource Center','Antarctia','Antartica','McMurdo Station','NASA WFF Site-wide',
...
fdt$Area[grepl(paste(paste0('\\b',c('Combat','Range Complex','Garrison Support','Military Training','Deployment',
                                    ...
                                    'Composite Wing Establishment','Range Safety Zones','Airfield Operations','Swimmer Interdiction','Interceptor',
                                    'Range Operations and Training','Operational Efficiency','Military Operations Area','Fighter Squadrons',
                                    'Test and Training Range','Aircraft Conversion','Enhanced Training Project','Training Range Development','Intelligence Surveillance',
                                    'Radar System','Fleet','Battle Course','Special Operations','Homeporting Facilities','Pinecastle Range',
                                    'Military Readiness','Bombing Range','Towed Array','Missile Range','Undersea Warfare','Structure Realignment','Mine Warfare',
...
fdt$Area[grepl(paste(paste0('\\b',c('Fishery Management','Rangeland Management','Vegetation','Watershed Improvement','Total Allowable Catch',
                                    ...
                                    'Landscape Area Treatments','Integrated Resource Project','Conservation Area Plan Amendments',
                                    ...
                                    'Reservoir Area Management Plan','National Preserve Public Access','Invasive Plant','Invasive Species','Analyses Area Amendment',
                                    'Resource Area Management Plan','California Desert Conservation Area Plan Amendments   Implementation','Wildlife Habitat Improvement Activities',
                                    'National Monument and Preserve Proposed Management Plan Amendment','Sanctuary Designation','Gold Butterfly','Monument Management Plans',
                                    ...
                                    'North Fork St. Joe River Project','Access Route on East Mountain','Protective Berm and Dune Construction','Recreation Area Master Plan','Designation and Management Plan',
                                    ...
                                    'Reservation Protection Project','Multi-Purpose Project','Recreation Management Area Plan','Storm Damage Risk Reduction','Shoreline Management Project','Comprehensive Framework for Managing Species and Habitats',
                                    ...
                                    'Forest Plan Riparian Amendment','Sevier River Resource Area','Ecosystem Analyses Area  Amendment','Forest Plan Amendments','Land Transfer','Breach Management Plan','Implementation Water Management Capability',
                                    ...
                                    'Shoreline Phase I','Beach Shoreline Management Project', 'Shoreline Restoration and Infrastructure Protection','Integrated Resource Project','SurgeProtection','Land and Resource Mgmt. Plan',
                                    'Habitat Preservation Area','National Wildlife Refuge Master Plan','Initial Stewardship Plan','Final CCP','REFUGE MASTER','Wildlife Refuge Act','Center Master Plan','Musselshell Analysis Area Implementation',
                                    'Riverway Cooperative Management  Plan','Recreation Area  Lake Management Plan','Rural Landscape Management Program ','Heritage Area Management Action Plan','Management Direction Implementation',
                                    'Jamaica Bay Reformulation','Adams and Denver Counties, Colorado General Investigation Study','Warm Springs Management Plan','Management Plan Implementation','Marine Sanctuaries Proposes',
                                    ...
                                    'National Monument Management  Plan  Implementation','Floodplain Area Study','Special Area Management Plan','Update and Consolidate Management Plans','\\bRMP\\b',
...
fdt$Area[grepl(paste(paste0('\\b',c('Conservation Plan','Habitat Conservation','Habitat Restoration','Protected Area',"Wilderness Area",'Wild and Scenic',
                                    ...
                                    'Restoration Enhancement','Invasive Plants','Northern Goshawk','Spotted Owl','Watershed Rehabilitation','Restoration of the Mariposa Grove','Ungulate Management Plan',
                                    ...
                                    'Enhancement of Threatened and Endangered Species','Reserve Enhancement Project','Dry Forest Restoration','Recovery Implementation Program','Habitat Areas of Particular Concern',
                                    'Salmon Restoration Activities','Reintroduction','Invasive Spartina','Reduce Predation','Double crested Cormorant','Eagle Rule Revision','Integrated Resource Plan',
                                    ...
                                    'Ballast Water Discharge','Marine Protected Species','Protected Living Marine Resource','Protect Adult Salmon','Basin Restoration Program','Conservation Strategy',
                                    'Wilderness Stewardship Plan','Conservation Reserve Program Implementation and Expansion','Ensure Viability','Management of Desert Tortoise Habitat','Seagrass Restoration',
                                    ...
                                    'Valley Rehabilitation','Watershed Protection Project','Winter Elk Management','Prairie Dog Conservation','Wildlife Mitigation Program','Habitat Enhancement',
                                    'Wetland Restoration Project','Non-Native','Watershed Protection Plan','Central and Southern Florida Project','Ragged Ruby Project',
...
fdt$Area[fdt$Agency == 'Community Development Block Grant'|grepl(paste(paste0('\\b',c('Business Development','Disposal and Reuse  Implementation','Property Exchange',
                                                                                      ...
                                                                                      'Plant Pests','Monterey Park Towne Plaza',
                                                                                      'Amoruso Ranch',
...
fdt$Area[grepl(paste(paste0('\\b',c('Campus Consolidation','FDA HQ','Campus Integration','Education Campus','Port of Entry Modernization',
                                    ...
                                    'Area B of the Presidio','Mail Facility Complex Development','medical Center','Closures Realignment','Camp Expansion and Realignment',
...
fdt$Area[grepl('Maglev',fdt$Title)] <- 'Science/Research'
fdt$Area[grepl("Sewer Project",fdt$Title)]<-'Water Works'
fdt$Area[grepl("Study and Combat Emerging Infectious Disease",fdt$Title)]<-'Science/Research'
...
built_indicators = c("I-[0-9]{1,3}|I[0-9]{1,3}|US(-|\\s)[0-9]{1,3}","SR(-|\\s)[0-9]{1,3}","S\\.R\\.\\s[0-9]{1,3}","Project","Completion","Facilit","Widening","Bypass","Extension","Special Use Permits","Bridge",
                     ...
                     "Special-Use-Permits",'Upgrade Proect',"Riley Ridge to Natrona",'North Fork Wells of Eagle Creek','Amoruso Ranch','River Locks','Bay Reformulation','Coastal Resiliency')
inst_indicators = c("Remedial","Program","Conservation","Maintenance","Activities","Protection",'Remediation','Transfer','Reintroduction','Rule-Making','Presence','Use of','Risk Reduction',
                    ...
                    'Integrated RMP','Special Recreation Permit',
...
#test = fread('input/epa_master_repository/eis_record_detail_coded.csv')
#top10 = test[,.N,by=.(Agency)][order(-N),][1:10,]$Agency
#table(test$Agency %in% top10,test$InfraType)


fwrite(fdt,'input/epa_master_repository/eis_record_detail_coded.csv')

# text_list = pblapply(which(!is.na(es_set$other_name)),function(x) {temp = fread(es_set$other_name[x]);temp$File = es_set$other_name[x];
# if(is.na(es_set$Exec_Summary_Start_Page[x])&is.na(es_set$Exec_Summary_End_Page[x])){temp}else{temp[Page %in% es_set$Exec_Summary_Start_Page[x]:es_set$Exec_Summary_End_Page[x],]}})
# text_dt = rbindlist(text_list,use.names = T,fill = T)
# text_dt$EIS.Number = str_extract(basename(text_dt$File),'^[0-9]{8}')
# saveRDS(text_dt,'scratch/descriptive_eis/eis_execsum_text.RDS')
text_dt = readRDS('scratch/descriptive_eis/eis_execsum_text.RDS')

es_set = fread('exec_summary_locations_hand.csv',na.strings = '')
es_set = es_set[!duplicated(es_set),]
es_set = es_set[order(is.na(Exec_Summary_File),-EIS.Number),][!duplicated(EIS.Number),]
es_set = es_set[DOCUMENT=='Final',]
es_set$Exec_Summary_File <- gsub('PDF$','pdf',es_set$Exec_Summary_File)
es_set$Exec_Summary_File <- gsub('\\.pd$','.pdf',es_set$Exec_Summary_File)
es_set$Exec_Summary_File <- gsub('_{1,}','_',es_set$Exec_Summary_File)

es_set$Exec_Summary_File[es_set$Exec_Summary_File=='']<-NA
es_set$Exec_Summary_File = str_squish(gsub('\\"','',es_set$Exec_Summary_File))
txfiles = list.files('../eis_documents/enepa_repository/text_as_datatable/',full.names = T,recursive = T)
es_set$other_name = gsub('pdf$|PDF$','txt',es_set$Exec_Summary_File)
es_set$other_name = gsub('\\bdocuments','text_as_datatable',es_set$other_name)
es_set$other_name <- textclean::replace_non_ascii(es_set$other_name,replacement = '-')
es_set$other_name <- gsub('_{2,}','_',es_set$other_name)
require(htmlTable)

es_set = es_set[EIS.Number %in% fdt$EIS.Number,]

sample_by_year = es_set[,list(sum(is.na(Exec_Summary_File)),sum(!is.na(Exec_Summary_File))),by=.(YEAR)]
setnames(sample_by_year,c('V1','V2'),c('No ES','ES'))
tab = htmlTable(sample_by_year[order(-YEAR,)])
sink(file = 'output/descriptive_eis/sample_by_year.html')
print(tab,type = 'html',useViewer = F)
sink()

es_set$Agency = epa$Agency[match(es_set$EIS.Number,epa$EIS.Number)]
sample_by_agency = es_set[,list(sum(is.na(Exec_Summary_File)),sum(!is.na(Exec_Summary_File))),by=.(Agency)]
setnames(sample_by_agency,c('V1','V2'),c('No ES','ES'))
tab = htmlTable::htmlTable(sample_by_agency[order(-ES,)])
sink(file = 'output/descriptive_eis/sample_by_agency.html')
print(tab,useViewer = F,type='html')
sink()


fdt2 = merge(fdt,es_set[,.(EIS.Number,Exec_Summary_File,Exec_Summary_Start_Page,Exec_Summary_End_Page)],all=T)

table(fdt2$YEAR,!is.na(fdt2$Exec_Summary_File))
feis_docs = fdt2[YEAR %in% 2013:2019,]
table(is.na(feis_docs$Exec_Summary_File),feis_docs$KEEP)

tab = htmlTable(fdt2[,.N,by=.(Area)][order(-N),])
sink(file = 'output/descriptive_eis/eis_by_area.html')
print(tab,useViewer = F,type='html')
sink()

tab = htmlTable(fdt2[,.N,by=.(InfraType)][order(-N),])
sink(file = 'output/descriptive_eis/eis_by_infratype.html')
print(tab,useViewer = F,type='html')
sink()


tab = htmlTable(fdt2[YEAR%in%2013:2019,.N,by=.(InfraType)][order(-N),])
sink(file = 'output/descriptive_eis/eis_by_infratype_subsample.html')
print(tab,useViewer = F,type='html')
sink()
