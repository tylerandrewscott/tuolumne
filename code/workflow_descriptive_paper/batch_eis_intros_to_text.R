...
crecord = drecord[!grepl('^APPENDIX|ATTACHMENT|COMMENT|^APPENDICE|^FIGURE|FIGURE|MAP[0-9]|ADDENDUM|^COVER SHEET',toupper(drecord$Original_File_Name)),]
...
extra_files_df = data.frame(FLOC = 'input/nepa_feis_externalfinds/',EIS.Number = str_extract(extra_files,'[0-9]{8}'), Original_File_Name = extra_files,File_Name = extra_files,stringsAsFactors = F)
...
#need_still = erecord$EIS.Number[!erecord$EIS.Number %in% fle]
...
matches = pblapply(seq_along(erecord$EIS.Number),function(i) {
  ...
  temp_read_list = lapply(seq_along(temp_flist$File_Name),function(f){
    ...
    intro_pages = which((!grepl(es_regex_exclude,pages_top3,perl=T)) & {sapply(pages,function(x) any(grepl('INTRODUCTION$|Introduction$|^Introduction|^INTRODUCTION',x)))|sapply(pages_top3,function(x) any(grepl('INTRODUCTION$|Introduction$|^Introduction|^INTRODUCTION',x)))|sapply(pages,function(x) any(grepl('(Page\\s|)1-[0-9]{1,3}$',x,perl=T)))}) 
    blanks = which(grepl('intentionally',tolower(temp_read$content)) & grepl('left blank',tolower(temp_read$content)))
     
    intro_pages = intro_pages[intro_pages < 100 | (intro_pages + 2) %in% intro_pages | (intro_pages - 2) %in% intro_pages |
                               (intro_pages + 1) %in% intro_pages | (intro_pages - 1) %in% intro_pages]
    intro_pages = unique(sort(c(intro_pages,which(((1:length(temp_read$content)) + 1) %in% intro_pages & ((1:length(temp_read$content)) -1) %in% intro_pages))))
    intro_pages = intro_pages[!intro_pages %in% blanks]
  ...
  if(length(temp_read_list)>0){
    temp_read_all = unlist(temp_read_list)
    ...
    {write.table(temp_read_all, file=paste0('input/eis_intro_text/',gsub('pdf','txt',paste0('Intro_',erecord$EIS.Number[i],'.txt'))),
                 quote = FALSE, row.names = FALSE, col.names = FALSE, eol = " ");rm(temp_read_all);rm(temp_read_list)}}
},cl = 4)
