...
all_sum_dt <- sum_dt
all_sum_dt[,grepl('P_',colnames(all_sum_dt))] <- all_sum_dt[,grepl('P_',colnames(all_sum_dt))]/all_sum_dt$tot_sents
  
plot_df = all_sum_dt %>% gather('Topic','Prop',-file,-type,-class_sents,-tot_sents)
plot_df$Topic = gsub('^P_','',plot_df$Topic)

plot_df$file <- gsub('_',' ',plot_df$file)
plot_df$file <- gsub('Oil Gas','Oil & Gas',plot_df$file)
plot_df = plot_df %>% arrange(file)
library(forcats)
plot_df$file <- fct_inorder(plot_df$file)
plot_df$file <- fct_rev(plot_df$file)
figure1 = ggplot(plot_df %>% filter(type=='FEIS'),aes(x = file,y = Prop,fill= Topic)) + 
  geom_bar(stat = 'identity')+
  scale_y_continuous(name = 'Proportion of tokenized sentences',limits=c(0,0.5),
                     expand= c(0,0)) + 
  scale_fill_colorblind()  + coord_flip() + 
  theme_bw() + scale_x_discrete(name = 'test') + ggtitle('Proportion of focus in final EISs')+
  theme(axis.title.y = element_blank(),axis.ticks = element_blank(),axis.text.y = element_text(face = 'bold'),
        text = element_text(family = 'Times'))
ggsave(figure1,filename = 'output/eiar/figure1.tiff',dpi = 500,units = 'in',height=6,width=6)
...
test = box_sums[box_sums$type == 'Comment',]
tt = aov(Value~Topic,test)
summary.aov(tt)
TukeyHSD(tt,ordered = T)
box_sums$Topic[box_sums$Topic=="Climate_Change"] <- 'Climate Change'
figure3 = ggplot(box_sums,aes(x = Topic,y = Value,color=type)) + geom_boxplot() + 
  scale_color_brewer(type = 'qual',palette = 2,name = 'Document') + theme_bw() + 
...
ggsave(figure3,filename = 'output/eiar/figure3.tiff',dpi = 500, width=6,height=4.5,units = 'in')
...
perc_point_changes$Topic[perc_point_changes$Topic=='Climate_Change'] <- 'Climate Change'
...
g1 = ggplot(perc_point_changes %>% filter(type == 'Comment') %>% select(-change_draft_to_final)) + 
  ...
  ggtitle("Change in content focus from draft EIS to comments")+ coord_flip()

g2 = ggplot(perc_point_changes %>% filter(type == 'FEIS') %>% select(-change_draft_to_final)) + 
  geom_boxplot(aes(x = Topic,y=change)) + theme_bw() + scale_y_continuous(name = "Percentage point change") + 
  ggtitle("Change in content focus from comments to final EIS")+ coord_flip()
...
g3 =  ggplot(perc_point_changes %>% filter(type == 'FEIS')) + 
  ...
  ggtitle("Change in content focus from draft EIS to final EIS") + coord_flip()

library(gridExtra)
figure4 = grid.arrange(g1 + theme(text = element_text(family = 'Times'),axis.title.y = element_blank()),
             g2 + theme(text = element_text(family = 'Times'),axis.title.y = element_blank()),
             g3 + theme(text = element_text(family = 'Times'),axis.title.y = element_blank()),ncol = 1)
ggsave(figure4,filename = 'output/eiar/figure4.tiff',dpi = 500, width=6,height=6,units = 'in')
...
radar_dt$file <- paste0(gsub('_',' ',radar_dt$file),' Project')
radar_dt$file[radar_dt$file== "Casa Diablo IV Geothermal Development Project"] <-  "Casa Diablo IV\nGeothermal Development"
radar_dt$file[radar_dt$file== "Barren Ridge Renewable Transmission Project"] <-  "Barren Ridge Renewable\nTransmission Project"
scales::show_col(viridis_pal()(8))
names(radar_dt)[names(radar_dt)=='Climate_Change'] <- 'Climate Change'
radar_list = lapply(sort(unique(radar_dt$file)),function(x) {
  ...
  tdf = radar_dt[radar_dt$file ==x,c('type',grep('^[A-Z]',colnames(radar_dt),value=T))]
    ggr <- ggradar(data.frame(tdf),grid.max = 0.6,grid.mid = 0.25,label.gridline.max = TRUE,label.gridline.mid=TRUE,label.gridline.min = FALSE,
                 values.radar = c(0.0,0.25,0.50),alpha = 0.7,point_type = 21,group.colours = (viridis_pal()(8))[c(1,5,8)],
                 axis.line.colour = 'transparent',background.circle.colour = 'white',
                 axis.label.size = 3,group.line.width = 0.5,plot.title = x,font.radar = 'Times',axis.labels = grep('^[A-Z]',colnames(radar_dt),value=T),
                 grid.label.size=3,legend.text.size = 30,group.point.size = 3,gridline.mid.colour = 'grey80',gridline.max.colour = 'grey80') +
    theme(text = element_text(family = 'Times'),
          legend.position = 'bottom',legend.text = element_text(size = 8),title = element_text(size = 10)) + 
      guides(colour = guide_legend(override.aes = list(alpha=1,lwd=1.5)))
  ggr 
})
library(gridExtra)
lgrob = g_legend(radar_list[[1]])

figure2 = grid.arrange(arrangeGrob(radar_list[[1]]+guides(color=FALSE),
                         radar_list[[2]]+guides(color=FALSE),
                         radar_list[[3]]+guides(color=FALSE),
                         radar_list[[4]]+guides(color=FALSE),nrow = 2),lgrob,nrow = 2,heights = c(10,1))
ggsave(figure2,filename = 'output/eiar/figure2.tiff',width=6,height=6,units = 'in',dpi = 500)                    
